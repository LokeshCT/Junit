...
public class OfferCreationValidatorTest {
    ...
    private OfferCreationValidator offerCreationValidator = new OfferCreationValidator(capabilityProvider, projectResource, productInstanceClient);
    ...
    @Test
    public void shouldValidateOfferCreationWhenBundleProductInvolved() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(true);
        ...
        //When
        OfferAndOrderValidationResult result = offerCreationValidator.validate("aProjectId", "aQuoteOptionId", newArrayList("bundleOwnerItemId", "bundleRelatedItemId"));
    ...
    @Test
    public void shouldValidateOfferCreationWhenBundleProductNotInvolved() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(true);
        when(quoteOptionItemResource.get()).thenReturn(newArrayList(nonBundleOwner, unRelated));

        //When
        OfferAndOrderValidationResult result = offerCreationValidator.validate("aProjectId", "aQuoteOptionId", newArrayList("nonBundleOwnerItemId"));
    ...
    @Test
    public void shouldNotValidateOfferCreationWhenInLifeSwitchNotEnabled() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(false);
    ...
    @Test
    public void shouldReturnValidationMessageWhenBundleOwnerIsNotSelectedInOfferCreation() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(true);
        ...
        //When
        OfferAndOrderValidationResult result = offerCreationValidator.validate("aProjectId", "aQuoteOptionId", newArrayList("bundleRelatedItemId"));

        //Then
        assertThat(result.isValid(), is(false));
        assertThat(result.getErrorMessage(), is("Please select bundle/proposition product along with currently selected items for offer creation and proceed"));
    }

    @Test
    public void shouldReturnValidationMessageWhenBundleRelatedIsNotSelectedInOfferCreation() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(true);
        when(quoteOptionItemResource.get()).thenReturn(newArrayList(owner, related));
        ...
        //When
        OfferAndOrderValidationResult result = offerCreationValidator.validate("aProjectId", "aQuoteOptionId", newArrayList("bundleOwnerItemId"));

        //Then
        assertThat(result.isValid(), is(false));
        assertThat(result.getErrorMessage(), is("Please select all related orderable products along with bundle product for offer creation and proceed"));

    }

    @Test
    public void shouldValidateMessageWhenBundleProductInvolvedInOfferCreation() {
        //Given
        when(capabilityProvider.isFunctionalityEnabled(USE_BUNDLE_PRODUCT_OFFER_VALIDATION, true, Optional.of("aQuoteOptionId"))).thenReturn(true);
