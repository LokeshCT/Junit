...
public class IndirectUserPricingSheetVisitor extends CompositeLineItemVisitor {
    ...
    public IndirectUserPricingSheetVisitor(ProductInstance productInstance, Map<String, Object> output,
    ...
    public void visitAfterChildren(LineItemModel lineItem) {
        ...
        if(productInstance.getProductOffering().isSiteInstallable()){
            ...
            output.put(SITE_OFFNET_USAGE_RRP, offnetUsage.toDouble());
            output.put(SITE_ONNET_USAGE_RRP, onnetUsage.toDouble());
            output.put(SITE_CHARGE_OT, oneTime.toDouble());
            output.put(COUNTRY, site.country);
            output.put(SITE_SW_NAME, "");
            output.put(SITE_SW_CHARGE_OT, 0.0);
            output.put(SITE_SW_CHARGE_MRR, 0.0);
            output.put(SITE_NSI_NAME, "");
            output.put(SITE_NSI_CHARGE_OT, 0.0);
            output.put(SITE_NSI_CHARGE_MRR, 0.0);
        }
        if(siteAgnosticMap!= null) {
            siteAgnosticMap.put(REVISION_TYPE, productInstance.getProductIdentifier().getVersionNumber());
            siteAgnosticMap.put(SITE_ONE_TIME_RRP, oneTime.toDouble());
            siteAgnosticMap.put(SITE_RECURRING_RRP, recurring.toDouble());
            siteAgnosticMap.put(CONTRACT_TERM, lineItem.getContractTerm());
            siteAgnosticMap.put(REMAINING_CONTRACT_TERM, lineItem.getContractTerm());
            siteAgnosticMap.put(SERVICEDATA_CHARGE_OT, 0);
            siteAgnosticMap.put(SERVICEDATA_CHARGE_MRR, 0);
            siteAgnosticMap.put(SERVICEDATA_STD_CHARGE_OT, 0);
            siteAgnosticMap.put(SERVICEDATA_STD_CHARGE_MRR, 0);
            siteAgnosticMap.put(SERVICEDATA_NSI_CHARGE_OT, 0);
            siteAgnosticMap.put(SERVICEDATA_NSI_CHARGE_MRR, 0);
        }
